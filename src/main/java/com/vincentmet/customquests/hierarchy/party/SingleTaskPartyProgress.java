...
public class SingleTaskPartyProgress extends HashMap<Integer, SingleSubtaskPartyProgress> implements IJsonObjectProcessor, IJsonObjectProvider{
	...
	public SingleTaskPartyProgress(int partyId, int questId, int taskId){
			...
			if(jsonElement.isJsonObject()){
				...
				for(Entry<String, JsonElement> jsonEntryElement : jsonObject.entrySet()){
					...
					if(value.isJsonObject()){
						...
						SingleSubtaskPartyProgress singleSubtaskProgress = new SingleSubtaskPartyProgress(partyId, questId, taskId, keyInt);
	...
	public JsonObject getJson(){
		...
		for(Entry<Integer, SingleSubtaskPartyProgress> entry : entrySet()) jsonEntries.add(entry.getKey().toString(), entry.getValue().getJson());
	...
	public void generateMissingValues(){
		QuestingStorage.getSidedQuestsMap().get(questId).getTasks().get(taskId).getSubtasks().forEach((subtaskId, subtask) -> {
			if(!containsKey(subtaskId)){
				put(subtaskId, new SingleSubtaskPartyProgress(partyId, questId, taskId, subtaskId));
			}
		});
		forEach((integer, singleSubtaskProgress) -> singleSubtaskProgress.generateMissingValues());
	}
