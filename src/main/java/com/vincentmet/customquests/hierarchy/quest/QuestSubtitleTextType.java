...
public class QuestSubtitleTextType implements IJsonObjectProcessor, IJsonObjectProvider, IEditorPage {
    private final int parentId;
    private ITextType type;

    public QuestSubtitleTextType(int parentId){
        ...
        if(json.has("type")){
            ...
            if(jsonElement.isJsonPrimitive()){
                ...
                if(jsonPrimitive.isString()){
                    ...
                    Ref.CustomQuests.LOGGER.warn("'Quest > " + parentId + " > subtitle > type': Value is not a String, defaulting to 'customquests:plaintext'!");
                    setType(new PlainTextTextType());
                }
            }else{
                Ref.CustomQuests.LOGGER.warn("'Quest > " + parentId + " > subtitle > type': Value is not a JsonPrimitive, please use a String, defaulting to 'customquests:plaintext'!");
                setType(new PlainTextTextType());
            }
        }else{
            Ref.CustomQuests.LOGGER.warn("'Quest > " + parentId + " > subtitle > type': Not detected, defaulting to 'customquests:plaintext'!");
            setType(new PlainTextTextType());
        }
        ...
        if(json.has("text")){
            ...
            if(jsonElement.isJsonPrimitive()){
                ...
                if(jsonPrimitive.isString()){
                    ...
                    Ref.CustomQuests.LOGGER.warn("'Quest > " + parentId + " > subtitle > text': Value is not a String, defaulting to an empty String!");
                }
            }else{
                Ref.CustomQuests.LOGGER.warn("'Quest > " + parentId + " > subtitle > text': Value is not a JsonPrimitive, please use a String, defaulting to an empty String!");
            }
        }else{
            Ref.CustomQuests.LOGGER.warn("'Quest > " + parentId + " > subtitle > text': Not detected, defaulting to an empty String!");
        }
    }
    ...
    public void setType(ITextType type){
        if(type != null){
            ...
            Ref.CustomQuests.LOGGER.warn("'Quest > " + parentId + " > subtitle > type': Value does not match a registered TextType or is null, please download the addon mod it belongs to, or change it to something valid. Defaulting to 'customquests:plaintext'");
            setType(new PlainTextTextType());
        }
    }
    ...
    public void addPageEntries(List<IEditorEntry> list) {
        list.add(new EditorEntryWrapper(new TextComponent("Type"), new ResourceLocation(Ref.MODID, "resourcelocation"), () -> type.getId().toString(), newValueObject -> {
            ...
            EditorGuiHelper.Update.Quest.Subtitle.requestUpdateType(parentId, getType().getId());
        }));
        list.add(new EditorEntryWrapper(new TextComponent("Text"), new ResourceLocation(Ref.MODID, "plaintext"), () -> type.getOgText(), newValueObject -> {
            ...
            EditorGuiHelper.Update.Quest.Subtitle.requestUpdateText(parentId, getOgText());
        }));
    }
}