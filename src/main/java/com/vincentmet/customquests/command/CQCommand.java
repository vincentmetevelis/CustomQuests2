...
public class CQCommand{
    public static void register(CommandDispatcher<CommandSource> dispatcher){
        dispatcher.register(LiteralArgumentBuilder.<CommandSource>literal(Ref.MODID)
                ...
                .executes(context->{
                        context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.use_subcommand"));
                        return 0;
                })
        );
        ...
        return Commands.literal("party")
                .then(Commands.literal("list")
                    .executes(context -> {
                        if(QuestingStorage.getSidedPartiesMap().size()>=1){
                            QuestingStorage.getSidedPartiesMap().values().forEach(party->context.getSource().sendSuccess(new StringTextComponent("ID: " + party.getId() + ", " + new TranslationTextComponent(Ref.MODID + ".general.name").getString() + ": " + party.getName()), false));
                        }else{
                            context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.list.no_parties"));
                        }
                        return 0;
                    })
                )
                        ...
                        .executes(context -> {
                            ...
                            if(Pattern.matches("[A-Za-z0-9]+?", name)){
                                ...
                                if(partyId == Ref.NO_PARTY){
                                    context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.create.error"));
                                }else{
                                    String devModeMessage = Config.SidedConfig.isDebugModeOn() ? " (ID: "+partyId+", name:'" + name + "')" : "";
                                    context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.party.create.success", devModeMessage), false);
                                    context.getSource().getServer().getPlayerList().getPlayers().forEach(ServerUtils::sendProgressAndParties);
                                }
                            }else{
                                context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.create.invalid_name"));
                            }
                            return 0;
                        })
                    )
                    .executes(context -> {
                        context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.create.no_name"));
                        return 0;
                    })
                )
                .then(Commands.literal("leave")
                    .executes(context -> {
                        ...
                        if(playerParty == Ref.NO_PARTY){
                            context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.leave.not_in_party"));
                            return 0;
                        }
                        if(PartyHelper.isPlayerPartyOwner(uuid, playerParty) && uuidsInParty.size() > 1){
                            context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.leave.party_owner"));
                            return 0;
                        }
                        if(uuidsInParty.size() == 1){ //If player is last one in party
                            ...
                            context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.party.delete.success"), false);
                            return 0;
                        }
                        ProgressHelper.setPlayerParty(uuid, Ref.NO_PARTY);
                        context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.leave.success"));
                        return 0;
                    })
                )
                    ...
                    .then(Commands.argument("new_owner", EntityArgument.player())
                        .executes(context -> {
                            ...
                            if(!ProgressHelper.isPlayerInParty(uuid)){
                                context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.set_owner.not_in_party"));
                            }else if(newOwner.getUUID().equals(uuid)){
                                context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.set_owner.already_owner"));
                                ...
                                context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.party.set_owner.success", newOwner.getDisplayName().getString()), false);
                            }else if(ProgressHelper.getPlayerParty(newOwner.getUUID()) == playerParty){
                                context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.set_owner.not_owner"));
                            }else{
                                context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.set_owner.new_owner_not_in_party"));
                            }
                            return 0;
                        })
                    )
                    .executes(context -> {
                        context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.set_owner.no_name"));
                        return 0;
                    })
                )
                .then(Commands.literal("invite")
                     .then(Commands.argument("player", EntityArgument.player())
                         .executes(context -> {
                             PlayerEntity inviter = context.getSource().getPlayerOrException();
                             ...
                             if(ProgressHelper.doesPlayerExist(inviterUUID) && ProgressHelper.isPlayerInParty(inviterUUID)){
                                 ...
                                 if(invitedPlayerUUID.equals(inviterUUID)){
                                     context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.invite.yourself"));
                                 }else if(PartyInviteCache.isPlayerInvitedToAnyParty(invitedPlayerUUID)){
                                     context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.invite.player_already_pending"));
                                 }else if(!PartyHelper.isPlayerPartyOwner(inviterUUID, partyId)){
                                     context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.invite.not_owner"));
                                 }else{
                                     PartyInviteCache.addInvite(invitedPlayerUUID, partyId);
                                     context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.party.invite.success.feedback", invitedPlayer.getDisplayName().getString()), false);
                     ...
                     .executes(context -> {
                         context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.invite.no_name"));
                         return 0;
                     })
                )
                .then(Commands.literal("accept")
                    .executes(context -> {
                        ...
                        if(ProgressHelper.isPlayerInParty(accepter)){
                            context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.accept.in_party"));
                        }else if(!PartyInviteCache.isPlayerInvitedToAnyParty(accepter)){
                            context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.accept.not_invited"));
                        }else{
                            int newPartyId = PartyInviteCache.getPartyForInvite(accepter);
                            ProgressHelper.setPlayerParty(accepter, newPartyId);
                            context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.party.accept.success"), false);
                        }
                        return 0;
                    })
                )
                .then(Commands.literal("deny")
                    .executes(context -> {
                        ...
                        if(ProgressHelper.isPlayerInParty(denier)){
                            context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.deny.in_party"));
                        }else if(PartyInviteCache.isPlayerInvitedToAnyParty(denier)){
                            context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.party.deny.not_invited"));
                        }else{
                            PartyInviteCache.removePlayerInvite(denier);
                            context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.party.deny.success"), false);
                        }
                        return 0;
                    })
                )
    ...
    public static ArgumentBuilder<CommandSource, ?> registerProgressCommand(){
        return Commands.literal("progress")
                ...
                .then(Commands.literal("delete")
                        .then(Commands.literal("all")
                                .executes(context -> {
                                    ...
                                    context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.progress.delete.all.success"), false);
                                    return 0;
                                })
                        )
                            ...
                            .then(Commands.argument("player_name", EntityArgument.players())
                                .executes(context -> {
                                    ...
                                    serverPlayerEntities.forEach(playerEntity -> {
                                        ...
                                        context.getSource().getServer().getPlayerList().getPlayers().forEach(ServerUtils::sendProgressToClient);
                                        context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.progress.delete.player.success", playerEntity.getDisplayName()), false);
                                    });
                                    return 0;
                                })
                            )
                ...
                .executes(context->{
                    context.getSource().sendFailure(new TranslationTextComponent(Ref.MODID + ".command.wip"));
                    return 0;
                })
        ;
    }
        ...
        return Commands.literal("settings")
                ...
                .then(Commands.literal("can_reward_only_be_claimed_once")
                     .then(Commands.argument("value", BoolArgumentType.bool())
                           .executes(context -> {
                               ...
                               context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.settings.set", "can_reward_only_be_claimed_once", TextFormatting.GOLD + String.valueOf(Config.SidedConfig.canRewardOnlyBeClaimedOnce())), false);
                               ServerUtils.sendServerConfigToAllPlayers();
                               return 0;
                           })
                     )
                     .executes(context -> {
                         context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.settings.get", "can_reward_only_be_claimed_once", TextFormatting.GOLD + String.valueOf(Config.SidedConfig.canRewardOnlyBeClaimedOnce())), false);
                         return 0;
                     })
                )
                .then(Commands.literal("edit_mode")
                     .then(Commands.argument("value", BoolArgumentType.bool())
                           .executes(context -> {
                               ...
                               context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.settings.set", "edit_mode", TextFormatting.GOLD + String.valueOf(Config.SidedConfig.isEditModeOnByDefault())), false);
                               ServerUtils.sendServerConfigToAllPlayers();
                               return 0;
                           })
                     )
                     .executes(context -> {
                         context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.settings.get", "edit_mode", TextFormatting.GOLD + String.valueOf(Config.SidedConfig.isEditModeOnByDefault())), false);
                         return 0;
                     })
                )
                .then(Commands.literal("give_device_on_first_login")
                     .then(Commands.argument("value", BoolArgumentType.bool())
                           .executes(context -> {
                               ...
                               context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.settings.set", "give_device_on_first_login", TextFormatting.GOLD + String.valueOf(Config.SidedConfig.giveDeviceOnFirstLogin())), false);
                               ServerUtils.sendServerConfigToAllPlayers();
                               return 0;
                           })
                     )
                     .executes(context -> {
                         context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.settings.get", "give_device_on_first_login", TextFormatting.GOLD + String.valueOf(Config.SidedConfig.giveDeviceOnFirstLogin())), false);
                         return 0;
                     })
                )
                .then(Commands.literal("debug_mode")
                     .then(Commands.argument("value", BoolArgumentType.bool())
                           .executes(context -> {
                               ...
                               context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.settings.set", "debug_mode", TextFormatting.GOLD + String.valueOf(Config.SidedConfig.isDebugModeOn())), false);
                               ServerUtils.sendServerConfigToAllPlayers();
                               return 0;
                           })
                     )
                     .executes(context -> {
                         context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.settings.get", "debug_mode", TextFormatting.GOLD + String.valueOf(Config.SidedConfig.isDebugModeOn())), false);
                         return 0;
                     })
                )
                .then(Commands.literal("backups")
                     .then(Commands.argument("value", BoolArgumentType.bool())
                           .executes(context -> {
                               Config.ServerConfig.GIVE_DEVICE_ON_FIRST_LOGIN = BoolArgumentType.getBool(context, "value");
                               context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.settings.set", "backups", TextFormatting.GOLD + String.valueOf(Config.SidedConfig.areBackupsEnabled())), false);
                               ServerUtils.sendServerConfigToAllPlayers();
                               return 0;
                           })
                     )
                     .executes(context -> {
                         context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.settings.get", "backups", TextFormatting.GOLD + String.valueOf(Config.SidedConfig.areBackupsEnabled())), false);
                         return 0;
                     })
                )
                .executes(context->{
    ...
    public static ArgumentBuilder<CommandSource, ?> registerInfoCommand(){
        return Commands.literal("info")
                .executes(context->{
                    context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.info.modid", Ref.MODID), false);
                    context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.info.mod_version", Ref.VERSION_MOD), false);
                    context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.info.mc_version", Ref.VERSION_MC), false);
                    return 0;
                })
        ;
    }
    ...
    public static ArgumentBuilder<CommandSource, ?> registerReloadCommand(){
        return Commands.literal("reload")
                ...
                .executes(context->{
                    ...
                    context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.reload.success"), false);
                    return 0;
                })
        ;
    }
    ...
    public static ArgumentBuilder<CommandSource, ?> registerUuidCommand(){
        return Commands.literal("uuid")
                .executes(context->{
                    context.getSource().sendSuccess(new TranslationTextComponent(Ref.MODID + ".command.uuid", context.getSource().getPlayerOrException().getStringUUID()), false);
                    return 0;
                })
        ;
    }
