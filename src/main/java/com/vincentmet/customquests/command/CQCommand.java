...
public class CQCommand{
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher){
        dispatcher.register(LiteralArgumentBuilder.<CommandSourceStack>literal(Ref.MODID)
                ...
                .executes(context->{
                        context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.use_subcommand"), false);
                        return 0;
                })
        );
    ...
    public static ArgumentBuilder<CommandSourceStack, ?> registerPartyCommand(){
        return Commands.literal("party")
                .then(Commands.literal("list")
                    .executes(context -> {
                        if(QuestingStorage.getSidedPartiesMap().size()>=1){
                            ...
                            context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.list.no_parties"), false);
                        }
                        return 0;
                    })
                )
                .then(Commands.literal("create")
                        ...
                        .executes(context -> {
                            ...
                            if(Pattern.matches("[A-Za-z0-9]+?", name)){
                                ...
                                if(partyId == Ref.NO_PARTY){
                                    context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.create.error"), false);
                                }else{
                    ...
                    .executes(context -> {
                        context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.create.no_name"), false);
                        return 0;
                    })
                )
                .then(Commands.literal("leave")
                    .executes(context -> {
                        ...
                        if(playerParty == Ref.NO_PARTY){
                            context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.leave.not_in_party"), false);
                            return 0;
                        }
                        if(PartyHelper.isPlayerPartyOwner(uuid, playerParty) && uuidsInParty.size() > 1){
                            context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.leave.party_owner"), false);
                            return 0;
                        }
                    ...
                    .then(Commands.argument("new_owner", EntityArgument.player())
                        .executes(context -> {
                            ...
                            if(!ProgressHelper.isPlayerInParty(uuid)){
                                context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.set_owner.not_in_party"), false);
                            }else if(newOwner.getUUID().equals(uuid)){
                                context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.set_owner.already_owner"), false);
                                ...
                                context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.set_owner.not_owner"), false);
                            }else{
                                context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.set_owner.new_owner_not_in_party"), false);
                            }
                            return 0;
                        })
                    )
                    .executes(context -> {
                        context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.set_owner.no_name"), false);
                        return 0;
                    })
                )
                .then(Commands.literal("invite")
                         ...
                         .executes(context -> {
                             ...
                             if(ProgressHelper.doesPlayerExist(inviterUUID) && ProgressHelper.isPlayerInParty(inviterUUID)){
                                 ...
                                 if(invitedPlayerUUID.equals(inviterUUID)){
                                     context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.invite.yourself"), false);
                                 }else if(PartyInviteCache.isPlayerInvitedToAnyParty(invitedPlayerUUID)){
                                     context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.invite.player_already_pending"), false);
                                 }else if(!PartyHelper.isPlayerPartyOwner(inviterUUID, partyId)){
                                     context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.invite.not_owner"), false);
                                 }else{
                     ...
                     .executes(context -> {
                         context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.invite.no_name"), false);
                         return 0;
                     })
                )
                .then(Commands.literal("accept")
                    .executes(context -> {
                        ...
                        if(ProgressHelper.isPlayerInParty(accepter)){
                            context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.accept.in_party"), false);
                        }else if(!PartyInviteCache.isPlayerInvitedToAnyParty(accepter)){
                            context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.accept.not_invited"), false);
                        }else{
                ...
                .then(Commands.literal("deny")
                    .executes(context -> {
                        ...
                        if(ProgressHelper.isPlayerInParty(denier)){
                            context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.deny.in_party"), false);
                        }else if(PartyInviteCache.isPlayerInvitedToAnyParty(denier)){
                            context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.party.deny.not_invited"), false);
                        }else{
    ...
    public static ArgumentBuilder<CommandSourceStack, ?> registerProgressCommand(){
        return Commands.literal("progress")
                ...
                .executes(context->{
                    context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.wip"), false);
                    return 0;
                })
        ;
    }
                ...
                .then(Commands.literal("edit_mode")
                     .then(Commands.argument("value", BoolArgumentType.bool())
                           .executes(context -> {
                               ...
                               context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.settings.set", "edit_mode", ChatFormatting.GOLD + String.valueOf(Config.SidedConfig.isEditModeOnByDefault())), false);
                               ServerUtils.sendServerConfigToAllPlayers();
                               return 0;
                           })
                     )
                     .executes(context -> {
                         context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.settings.get", "edit_mode", ChatFormatting.GOLD + String.valueOf(Config.SidedConfig.isEditModeOnByDefault())), false);
                         return 0;
                     })
                )
    ...
    public static ArgumentBuilder<CommandSourceStack, ?> registerUuidCommand(){
        return Commands.literal("uuid")
                .executes(context->{
                    context.getSource().sendSuccess(new TranslatableComponent(Ref.MODID + ".command.uuid", context.getSource().getPlayerOrException().getUUID().toString()), false);
                    return 0;
                })
        ;
    }
