...
import com.vincentmet.customquests.gui.elements.TextButton;
...
public class DeliveryScreen extends Screen {
	public static final IntSupplier MENUS_BUTTON_WIDTH = ()->50;
	public static final IntSupplier MENUS_BUTTON_HEIGHT = ()->20;
	public final IntSupplier SELECT_BUTTON_WIDTH = ()->((width>>2)-30-40);
	public static final IntSupplier SELECT_BUTTON_HEIGHT = ()->20;
	...
	private VariableSlot selectSlot = new VariableSlot(0, 55, SLOT_SIZE.getAsInt(), SLOT_SIZE.getAsInt(), VariableSlot.SlotTexture.DEFAULT, (mouseButton)->{}, new ArrayList<>());
	...
	@Override
	protected void init(){
		questsList = new ScrollableList(questListX, questListY, questListWidth, questListHeight);
		taskList = new ScrollableList(taskListX, taskListY, taskListWidth, taskListHeight);
		subtaskList = new ScrollableList(subtaskListX, subtaskListY, subtaskListWidth, subtaskListHeight);
		reInit();
	}
	...
	private void reInit(){
		...
		IntCounter cumulativeHeight = new IntCounter(20, MENUS_BUTTON_HEIGHT.getAsInt());
	...
	private void reInitSubtaskItems(){
		...
		if(currentSubtask.getLeft()>=0 && currentSubtask.getMiddle()>=0){
			IntCounter cumulativeHeight = new IntCounter(0, MENUS_BUTTON_HEIGHT.getAsInt());
			QuestingStorage.getSidedQuestsMap().get(currentSubtask.getLeft()).getTasks().get(currentSubtask.getMiddle()).getSubtasks().entrySet()
			               ...
			               .forEach(entry -> {
			                   subtaskList.add(new TextButton(subtaskListX, ()->subtaskListY.getAsInt() + cumulativeHeight.getValue(), ()->(width>>2) - 20, MENUS_BUTTON_HEIGHT, entry.getValue().getSubtask().getText(clientPlayer), ButtonState.NORMAL, (mouseButton1)->{
	...
	public void render(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks){
		...
		if(currentSubtask.getLeft()>=0 && currentSubtask.getMiddle()>=0 && currentSubtask.getRight()>=0){
			...
			String checkmarkText = ChatFormatting.DARK_GRAY + " [" + QuestingStorage.getSidedPlayersMap().get(clientPlayer.getStringUUID()).getIndividualProgress().get(currentSubtask.getLeft()).get(currentSubtask.getMiddle()).get(currentSubtask.getRight()).getValue() + "/" + QuestingStorage.getSidedQuestsMap().get(currentSubtask.getLeft()).getTasks().get(currentSubtask.getMiddle()).getSubtasks().get(currentSubtask.getRight()).getSubtask().getCompletionAmount() + "]";
			if(isCompleted)
