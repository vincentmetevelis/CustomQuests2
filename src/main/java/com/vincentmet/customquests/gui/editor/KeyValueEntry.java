...
public class KeyValueEntry implements CQGuiEventListener, IHoverRenderable{
    ...
    private final IntSupplier SAVE_X = ()->x.getAsInt() + width.getAsInt() - SAVE_WIDTH.getAsInt();
    ...
    private final IntSupplier TEXTFIELD_WIDTH = ()->width.getAsInt()/3*2 - 5 - SAVE_WIDTH.getAsInt();
    ...
    public KeyValueEntry(EditorScreenManager screenManager, IEditorEntry editorEntry, IntSupplier x, IntSupplier y, IntSupplier width, IntSupplier height){
        ...
        this.save = new VariableButton(SAVE_X, SAVE_Y, SAVE_WIDTH, SAVE_HEIGHT, VariableButton.ButtonTexture.DEFAULT_NORMAL, "S", new Vec2i(), mouseButton -> {
            fieldType.save(editorEntry);
            ClientUtils.reloadEditorIfOpen();
        }, new ArrayList<>());//todo add buttonTexture changer
    }
