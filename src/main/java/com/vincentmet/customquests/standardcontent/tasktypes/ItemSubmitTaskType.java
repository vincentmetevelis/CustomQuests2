...
public class ItemSubmitTaskType implements ITaskType, IItemStacksProvider{
	...
	public void executeSubtaskCheck(PlayerEntity player, Object object){
		...
		player.inventory.mainInventory.forEach(invStack -> {
			items.stream().filter(itemStack -> itemStack.getItem() == invStack.getItem()).forEach(itemStack -> {
	...
	public ITaskButtonClick executeSubtaskButton(){
		return (player, questId, taskId, subtaskId) -> {
	...
	public void onLoad(UUID uuid, int questId, int taskId, int subtaskId){
		...
		StandardContentProgressHelper.ITEM_SUBMIT_SUBTASKS_TO_CHECK.add(new Quadruple<>(uuid, questId, taskId, subtaskId));
	}
