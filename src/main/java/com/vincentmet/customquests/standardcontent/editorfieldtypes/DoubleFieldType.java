...
public class DoubleFieldType implements IEditorEntryDataType {
    private final ResourceLocation ID = new ResourceLocation(Ref.MODID, "double");
    ...
    public void init(int x, int y, int width, int height, Object initialValue){
        ...
        this.rlValue = new SingleLineTextField(x, y, width, height, 0xFF000000, 0xFFAAAAAA, 0xFFFFFFFF, 0xFFFFFFFF, initialValue.toString(), Pattern.compile("^\\d*\\.?\\d*$"));//todo use this regex to check on saving in the editor, not on every character typed /// ^(-?)(0|([1-9][0-9]*))(\.[0-9]+)?$
    }
